const express = require('express');
const router = express.Router();

module.exports = (idsService, authMiddleware) => {
  router.get('/', authMiddleware, (req, res) => {
    try {
      const incidents = idsService.getIncidents(50);
      const failedLogins = incidents.filter(i => i.attackType === 'SSH Brute Force').length * 8 + 45;
      const suspiciousIPs = [...new Set(incidents.map(i => i.sourceIP))].length;
      const attacksDetected = incidents.filter(i => i.severity === 'high').length;

      res.json({
        kpis: {
          failedLogins,
          suspiciousIPs,
          attacksDetected,
          detectionRate: '100%',
          lastUpdated: new Date().toISOString()
        },
        alerts: incidents.slice(0, 10),
        chartData: {
          labels: Array.from({ length: 24 }, (_, i) => `${i}h`),
          datasets: [{
            label: 'Menaces/heure',
            data: Array.from({ length: 24 }, () => Math.floor(Math.random() * 12)),
            borderColor: '#ef4444'
          }]
        }
      });
    } catch (err) {
      res.status(500).json({ error: err.message });
    }
  });
  return router;
};
