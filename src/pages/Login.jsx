  ...
  return (
    <div style={{ minHeight: '100vh', background: 'linear-gradient(135deg, #0f172a 0%, #1e293b 100%)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: "'Inter', sans-serif" }}>
      <div style={{ background: 'rgba(30, 41, 59, 0.8)', padding: '40px', borderRadius: '12px', border: '1px solid rgba(148, 163, 184, 0.2)', width: '100%', maxWidth: '400px' }}>
        <h1 style={{ textAlign: 'center', color: '#f1f5f9', marginBottom: '8px', fontSize: '28px', fontWeight: '700' }}>Holberton SOC</h1>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <button type="submit" disabled={loading}
            style={{ width: '100%', padding: '12px', background: '#10b981', color: '#fff', border: 'none', borderRadius: '6px', cursor: loading ? 'not-allowed' : 'pointer', fontWeight: '600', fontSize: '14px', opacity: loading ? 0.7 : 1 }}>
            {loading ? 'Loading...' : 'Login'}
          </button>
        </form>

        <p style={{ textAlign: 'center', color: '#64748b', marginTop: '20px', fontSize: '12px' }}>
          analyst@holberton.io / Holberton2024!
        </p>
      </div>
    </div>
  );
